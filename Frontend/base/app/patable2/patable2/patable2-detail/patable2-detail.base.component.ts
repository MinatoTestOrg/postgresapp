...
export class Patable2DetailBaseComponent{
...
wfNotInitiated:boolean = false;
  ...
  "queryViewMapping" : { }
	...
	mapConfig : any = { }
	pageViewTitle: string = 'PATABLE2_DETAIL';
...
dynamicDialogRef:any;
    ...
    step:workflowInfo.step
           ...
           const methodName: any = "configureFormOnWorkflow";
            let action: Exclude<keyof Patable2DetailBaseComponent, ' '> = methodName;
...
onChangelog() {
	...
	editFormula(config:any,fieldname:any){
         ...
         if(fieldConfig.formulaMethod){
           let methodName: Exclude<keyof Patable2DetailBaseComponent, ' '> = fieldConfig.formulaMethod
  ...
  camelize(str: string) {
    ...
    if (this.checkValidations()) {
      ...
      if (Object.keys(splittedData.files).length > 0) {
        ...
        const saveSubscription = this.patable2Service[method](requestedObj, actionName).subscribe((res: Patable2Base) => {
          this.onAfterSave(res, data, method, isToastNotNeeded);
        })
        this.subscriptions.push(saveSubscription);
      }
...
uploadAttachmentsandSaveData(data: any, splittedData: any,actionName?:string): Observable<any> {
    ...
    if (!this.id) {
       const saveSubscription =  this.patable2Service.create(splittedData.data,actionName).subscribe(
        createdData => {
    ...
    const subject$ = new Subject();    
      const updateSubscription = this.deleteFiles(splittedData).subscribe((responseData: any) => {
        this.saveFiles(responseData).subscribe((dataToUpdate: any) => {
          ...
          this.patable2Service.update(dataToUpdate.data,actionName).subscribe(
            res => {
              subject$.next(res);
              subject$.complete();
            },
...
workflowInformation.step = workflowDataForNewRec.step || '';
  ...
  if (environment.prototype && this.id) {
    ...
    this.patable2Service.getProtoTypingDataById(params).subscribe((res: any) => {
    ...
    const params = {
      sid: this.id
        ...
        const dataSubscription = this.patable2Service.getById(params).subscribe((res: Patable2Base[]) => {
          this.data = res || {};
