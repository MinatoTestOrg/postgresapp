import { Patable1Service } from '../patable1.service';
import { Patable1Base} from '../patable1.base.model';
...
import { Patable1ApiConstants } from '@baseapp/patable1/patable1/patable1.api-constants';
...
export class Patable1DetailBaseComponent{
...
wfNotInitiated:boolean = false;
  ...
  "queryViewMapping" : { }
	...
	mapConfig : any = { }
	pageViewTitle: string = 'PATABLE1_DETAIL';
	
	public patable1Service = inject(Patable1Service);
...
dynamicDialogRef:any;
    ...
    let action: Exclude<keyof Patable1DetailBaseComponent, ' '> = methodName;
...
onChangelog() {
    ...
    if (this.checkValidations()) {
      ...
      if (Object.keys(splittedData.files).length > 0) {
        ...
        const saveSubscription = this.patable1Service[method](requestedObj, actionName).subscribe((res: Patable1Base) => {
          this.onAfterSave(res, data, method, isToastNotNeeded);
        })
        this.subscriptions.push(saveSubscription);
      }
...
uploadAttachmentsandSaveData(data: any, splittedData: any,actionName?:string): Observable<any> {
    ...
    if (!this.id) {
       const saveSubscription =  this.patable1Service.create(splittedData.data,actionName).subscribe(
        createdData => {
    ...
    const subject$ = new Subject();    
      const updateSubscription = this.deleteFiles(splittedData).subscribe((responseData: any) => {
        this.saveFiles(responseData).subscribe((dataToUpdate: any) => {
          ...
          this.patable1Service.update(dataToUpdate.data,actionName).subscribe(
            res => {
              subject$.next(res);
              subject$.complete();
            },
...
workflowInformation.step = workflowDataForNewRec.step || '';
  ...
  if (environment.prototype && this.id) {
    ...
    this.patable1Service.getProtoTypingDataById(params).subscribe((res: any) => {
    ...
    const params = {
      sid: this.id
        ...
        const dataSubscription = this.patable1Service.getById(params).subscribe((res: Patable1Base[]) => {
          this.data = res || {};
  ...
  if (this.appUtilBaseService.isEqualIgnoreCase(this.backupData, this.detailFormControls.getRawValue(), this.appUtilBaseService.getIgnorableFields(this.detailFormControls.getRawValue(),this.backupData,this.detailPageIgnoreFields), true)) {
	...
	editFormula(config:any,fieldname:any){
         ...
         if(fieldConfig.formulaMethod){
           let methodName: Exclude<keyof Patable1DetailBaseComponent, ' '> = fieldConfig.formulaMethod
	...
	getActorLabel(actor:string){
                const formattedActorName = actor.replace(/([a-z])([A-Z])/g, '$1_$2').toUpperCase();
                        ...
                        const field = this.detailFormConfig.children.find((child: any) => child.label === formattedActorName);
                         return field ? this.translateService.instant(field.label) : this.translateService.instant(formattedActorName);
                        }
          }
