...
export class Patable1ListBaseComponent{
...
localStorageStateKey = "patable1-list";
...
@Input() standardGrid:boolean = false;
  ...
  "sortOrder" : "asc",
  "detailPage" : {
    "sid" : "1b232bef-a396-4c0f-b0b5-9068a705a220",
    "name" : "paTable1 Detail",
    "url" : "/patable1/patable1detail"
  },
  "detailPageNavigation" : "click_of_the_row"
}
	pageViewTitle: string = 'PATABLE1_LIST';
...
focus(){
  ...
  disablechildAction(pid?:any) {
      ...
      let property: Exclude<keyof Patable1ListBaseComponent, ' '> = value;
  ...
  hasNumbers.forEach((f: any) => {
    ...
    if (this.selectedValues.length > 0) {
      ...
      this.confirmationService.confirm({
        ...
        accept: () => {
          const deleteSubscription = this.patable1Service.delete(requestedParams).subscribe((res: any) => {
  ...
  return date.getFullYear() + '-' + this.leftPad((date.getMonth() + 1), 2) + '-' + this.leftPad(date.getDate(), 2);
    ...
    let action: Exclude<keyof Patable1ListBaseComponent, ' '> = methodName;
...
leftPad(num:number, length:number) {
  ...
  if (defaultFilterField.operator?.toLowerCase()?.trim() === 'is empty') {
    ...
    if (environment.prototype && this.tableConfig.children?.length > 0) {
      gridSubscription = this.patable1Service.getProtoTypingData().subscribe((data: any) => {
 ...
 assignTableParams() {
  ...
  this.showAdvancedSearch = !this.showAdvancedSearch;
    ...
    return {
      ...
      ajaxUrl: Patable1ApiConstants.getDatatableData,
      select: true,
