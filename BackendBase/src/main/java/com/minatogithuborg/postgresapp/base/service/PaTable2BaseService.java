...
import org.apache.commons.lang3.StringUtils;
public class PaTable2BaseService<T extends PaTable2Base> extends BaseJPABusinessLogic<T>
		implements IPaTable2BaseService<T> {
		
	private static final Logger LOGGER = LoggerFactory.getLogger(PaTable2BaseService.class.getName());
	...
	@Autowired
	private  PaTable2PerimeterBaseImpl patable2PerimeterBaseImpl;
	
	
	public PaTable2BaseService(Class<T> modelClass) {
		super(modelClass);
	...
	public void onAfterSaveDB(T modelObj) {
	  changelogService.createChangeLog("PaTable2", modelObj.getSid(), Constants.SAVED, modelObj);
	}
	...
	public void onAfterUpdateDB(T modelObj) {
	  changelogService.createChangeLog("PaTable2", modelObj.getSid(), Constants.UPDATED, modelObj);
	}
	...
	public void onAfterDeleteDB(T modelObj) {
	  changelogService.createChangeLog("PaTable2", modelObj.getSid(), Constants.DELETED, modelObj);
	}	
	...
	protected IPerimeterManager<T> getPerimeterManager() {
		return patable2PerimeterBaseImpl;
	}
}