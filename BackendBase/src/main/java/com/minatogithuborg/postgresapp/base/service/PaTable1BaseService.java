package com.minatogithuborg.postgresapp.base.service;
...
import com.minatogithuborg.postgresapp.base.model.PaTable1Base;
import java.util.List;
...
import org.apache.commons.lang3.StringUtils;
public class PaTable1BaseService<T extends PaTable1Base> extends BaseJPABusinessLogic<T>
		implements IPaTable1BaseService<T> {
		
	private static final Logger LOGGER = LoggerFactory.getLogger(PaTable1BaseService.class.getName());
	...
	@Autowired
	private  PaTable1PerimeterBaseImpl patable1PerimeterBaseImpl;
	
	
	public PaTable1BaseService(Class<T> modelClass) {
		super(modelClass);
	...
	public void onAfterSaveDB(T modelObj) {
	  changelogService.createChangeLog("PaTable1", modelObj.getSid(), Constants.SAVED, modelObj);
	}
	...
	public void onAfterUpdateDB(T modelObj) {
	  changelogService.createChangeLog("PaTable1", modelObj.getSid(), Constants.UPDATED, modelObj);
	}
	...
	public void onAfterDeleteDB(T modelObj) {
	  changelogService.createChangeLog("PaTable1", modelObj.getSid(), Constants.DELETED, modelObj);
	}	
	...
	protected IPerimeterManager<T> getPerimeterManager() {
		return patable1PerimeterBaseImpl;
	}
}