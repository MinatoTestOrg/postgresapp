...
import com.minatogithuborg.postgresapp.base.model.constants.ApplicationUserConstantBase;
	...
	public boolean canCreate(T model) {
		...
		try {
			...
			if (userBase != null && CollectionUtils.isNotEmpty(userBase.getUserRoles())) {
				...
				throw new InternalException("Application user or role not found while verify create perimeter");
			}
	...
	public boolean canUpdate(T model) {
		...
		try {
			...
			if (userBase != null && CollectionUtils.isNotEmpty(userBase.getUserRoles())) {
				...
				throw new InternalException("Application user or role not found while verify update perimeter");
			}
		}catch (InternalException e){
			LOGGER.error("Exception :{0}",e);
			throw new InternalException(ErrorCode.APPLICATION_USER_ROLE_NOT_EXIST,e);
	...
	public boolean canDelete(T model) {
		...
		try {
			...
			if (userBase != null && CollectionUtils.isNotEmpty(userBase.getUserRoles())) {
				...
				throw new InternalException("Application user or role not found while verify delete perimeter");
			}
		}catch (InternalException e){
			LOGGER.error("Exception :{0}",e);
			throw new InternalException(ErrorCode.APPLICATION_USER_ROLE_NOT_EXIST,e);
	...
	public boolean canRead(T model) {
		...
		try {
			...
			if (userBase != null && CollectionUtils.isNotEmpty(userBase.getUserRoles())) {
				...
				throw new InternalException("Application user or role not found while verify read perimeter");
			}
		}catch (InternalException e){
			LOGGER.error("Exception :{0}",e);
			throw new InternalException(ErrorCode.APPLICATION_USER_ROLE_NOT_EXIST,e);
